...
:: PasteParty Update Pusher Script (Windows Version)
:: This script helps push updates to the develop branch of the PasteParty repository

:: Main script starts here
echo Starting PasteParty Update Process...
echo.

:: Check if we're in a git repository
if not exist ".git" (
    echo [ERROR] Not in a git repository. Please run this script from the PasteParty project directory.
    pause
    exit /b 1
)
...
git remote set-url origin https://github.com/richard1912/PasteParty.git
if errorlevel 1 (
...
if errorlevel 1 (
    ...
    if "!git_name!"=="" (
        echo [ERROR] Git user name is required. Exiting.
        pause
        exit /b 1
    )
    git config user.name "!git_name!"
    echo [INFO] Git user name set to: !git_name!
)
...
if errorlevel 1 (
    ...
    if "!git_email!"=="" (
        echo [ERROR] Git user email is required. Exiting.
        pause
        exit /b 1
    )
    git config user.email "!git_email!"
    echo [INFO] Git user email set to: !git_email!
)
echo.
...
echo [INFO] Repository: https://github.com/richard1912/PasteParty.git
